...
token_expiry_date_in = "Inventory.access_token_token_expiry_date_in"


def random_token(length=40):
    # we can agree here how we can manage the token?
    rbytes = os.urandom(length)
    test = "{}".format(str(hashlib.sha1(rbytes).hexdigest()))
    print ('--------------call-------call------------------ffffffff------------',test)
    return test
...
class APIAccessToken(models.Model):
    ...
    def find_or_create_token(self, user_id=None, create=False):
        ...
        print ('-----access_token-------------------vvvv-------------------',access_token)
        if access_token:
            print('----1------')
